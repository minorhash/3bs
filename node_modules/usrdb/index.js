...
exports.allNow=function(email){
var stm=db.prepare('select *,rowid from now where email=? order by rowid desc ');
...
exports.insPid=function(email,pid,mnt,ite,date,ship){
var stm=db.prepare('insert into pid values(?,?,?,?,?,?)');
stm.run(email,pid,mnt,ite,date,ship)
};
...
exports.insPal=function(email,id,ite,date){
var stm=db.prepare('insert into pal values(?,?,?,?)');

try{stm.run(email,id,ite,date)}
catch(err){console.log(err)}
};
